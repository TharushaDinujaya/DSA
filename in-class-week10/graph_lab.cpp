...
struct Graph{
    ...
    void addedge(int u, int v){
        ...
        Node * node = &(nodes)[u-1]; // list index = label value - 1
        (*node).neighbours.push_back(v);
        //select node v and push u into v's neighbour
        node = &(nodes)[v-1]; // list index = label value - 1
    ...
    void print(){
        ...
        for(int i = 0; i < n; i++){
            ...
            cout << "Label : " << (i+1) << " neighbours : [ ";
            list<int> l1 = nodes[i].neighbours;
            for( int neighbour : l1){ //prints neighbors in selected node
                cout << neighbour << ", ";
            }
            cout << "\b\b ]" << endl;
        }
